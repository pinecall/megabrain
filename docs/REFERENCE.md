...
|---|---|---|
| **`megabrain_ask`** | UNDERSTAND: a narrated walkthrough of the whole relevant flow with the real code spliced in verbatim — the code cannot be invented; the prose is narration, so check it against the code it quotes. | `query` *(req)* · `scope_path` · `content` |
| **`megabrain_code`** | CHANGE: the **edit surface** — every file to touch, the exact line, the surrounding code quoted verbatim, the new lines, and the neighbouring test to imitate. The engine opens files itself (`open_file`), so this is one call rather than one to understand plus one to locate. Same verb as `ask`, opposite deliverable; two tools because a tool NAME is what a model chooses by. | `task` *(req)* · `scope_path` |
