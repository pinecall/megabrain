...
| **`megabrain_node`** | ONE FILE'S PLACE in the repository — the half that reading the file cannot tell you. Every **dependant** and dependency with the kind of each edge, the cluster it belongs to, its **semantic twins** (files that do the same job and never import it), and the declared symbols with real line ranges. No code, on purpose. Dependants are split from **pins** (a test fixing the behaviour is not code that breaks), source leads tests and examples, and the file's own package leads the rest. ⚠️ For a language megabrain extracts no import graph for (everything but Python and TypeScript/JavaScript) the edge sections say **`not extracted`**, never `none` — absence there is not evidence. | `file` *(req)* · `label` *(default `false`)* |
