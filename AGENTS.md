...
| `indexing/` | `indexer` (incremental by content hash — **no auto-refresh**, at query time or otherwise) · `strategies` (ext → registry, the OCP point, `EDGE_SCHEMA`) · `passes/` plan→embed→write→resymbol · `edges/` per-language extractors (python · typescript · ruby · go · php, each an `EdgeLane` ROW in `_languages.py`) + `pins` · `_gitignore` |
...
| `graph/` | the graph (candidates + annotations, **never ranking**). `render` — one file's place as text, for `megabrain_node` · `weights`/`semantic`/`aliases` — what an edge weighs · `clusters/` communities and their labels — *the package's only LLM touch* · `routes/` BFS questions asked at query time · `symbols/` go-to-definition and its inverse |
...
| `transports/` | `cli` (one module per verb) · `mcp` (five tools — `node` is one file's dependants/cluster/twins; `inputSchema` GENERATED from `contracts/tools/`) · `http` (studio + JSON API, `ui/` is the built studio bundle) · `install` (`megabrain install` — the six-assistant MCP registration table) |
