...
| `transports/` | `cli` (one module per verb) · `mcp` (four tools; `inputSchema` GENERATED from `contracts/tools/`) · `http` (studio + JSON API, `ui/` is the built studio bundle) · `install` (`megabrain install` — the six-assistant MCP registration table) |
